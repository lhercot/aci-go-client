...
func (c *Client) Do(req *http.Request) (*container.Container, *http.Response, error) {
	...
	for attempts := 0; ; attempts++ {
		log.Printf("[DEBUG] HTTP Request Method and URL: %s %s", req.Method, req.URL.String())
		if !c.skipLoggingPayload {
			log.Printf("[DEBUG] HTTP Request Body: %v", req.Body)
		}
		...
		if !c.skipLoggingPayload {
			...
			log.Printf("[DEBUG] HTTP Response: %d %s", resp.StatusCode, resp.Status)
		}
...
func (c *Client) backoff(attempts int) bool {
	...
	log.Printf("[TRACE] Starting sleeping for %v seconds", backoff)
