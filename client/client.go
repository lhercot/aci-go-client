...
const DefaultBackoffDelayFactor int = 3
...
func BackoffDelayFactor(backoffDelayFactor int) Option {
	return func(client *Client) {
...
func (c *Client) backoff(attempts int) bool {
	...
	if c.backoffMinDelay != nil {
		timeout = time.Duration(c.backoffMinDelay) * time.Second
	}

	maxDelay := time.Duration(DefaultBackoffMaxDelay) * time.Second
	if c.backoffMaxDelay != nil {
		timeout = time.Duration(c.backoffMaxDelay) * time.Second
	}

	factor := DefaultBackoffDelayFactor
	if c.backoffDelayFactor != nil {
		timeout = c.backoffDelayFactor
	}
